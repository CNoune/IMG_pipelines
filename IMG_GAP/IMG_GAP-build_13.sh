...
LOGFILE=$IMG_dir/IMG_GAP/Logs/IMG_GAP_Logging.$FILEDATETIME.log
...
start:
	echo "Hello, "$USER". Welcome to IMG-GAPv2. Press [ENTER] to begin."
	read enter
	echo "Note: This Pipeline uses fastx-toolkit to complete trimming and is designed for single-end reads. If you use paired-end reads do not complete trimming with IMG-GAP. The pipeline assumes bbmap, kentUtils and biostar175929 are in the IMG_pipelines directory. If you wish to continue press [ENTER] otherwise press [ctrl] + [c] to exit."
	read enter
...
extract_seq:
	...
	do
		...
		do			
			...
			Rscript $IMG_dir/IMG_GAP/IMG_GAPv2_Back-ends/Seq_List.R $seq_list_dir $subset
			rm $subset
			rm Subset_stats.csv
			Rscript $IMG_dir/IMG_GAP/IMG_GAPv2_Back-ends/Subset_Stats.R $stat_merge_dir
...
finish:
echo "$USER you have now successfully completed IMG_GAPv2. Your final fasta list can now be used for further analysis (including taxa assignment using NCBI Blast) and relative abundance of each Taxa/OTU can be calculated from the csv file. Thank-you and don't forget to cite the method and software used."
exit 0
done		
